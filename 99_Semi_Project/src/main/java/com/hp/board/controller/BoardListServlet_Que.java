...
@WebServlet("/board/listQue")
public class BoardListServlet_Que extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		boardCount = service.getBoardQueCount(searchMap);
...
//		pageInfo = new PageInfo(page, 10, boardCount, 20); // 게시글이 20개로 보인다.
		list = service.getBoardQueList(pageInfo, searchMap);
...
//		list = service.getBoardList(pageInfo);
		...
		req.getRequestDispatcher("/views/html/3_boardList_Que.jsp").forward(req, resp);
	}
