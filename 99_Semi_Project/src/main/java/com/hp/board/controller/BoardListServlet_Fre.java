...
@WebServlet("/board/listFre")
public class BoardListServlet_Fre extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		boardCount = service.getBoardFreCount(searchMap);
...
//		pageInfo = new PageInfo(page, 10, boardCount, 20); // 게시글이 20개로 보인다.
		list = service.getBoardFreList(pageInfo, searchMap);
...
//		list = service.getBoardList(pageInfo);
		...
		req.getRequestDispatcher("/views/html/3_boardList_Fre.jsp").forward(req, resp);
	}
