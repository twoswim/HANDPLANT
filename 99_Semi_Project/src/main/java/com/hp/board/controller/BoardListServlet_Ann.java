...
@WebServlet("/board/listAnn")
public class BoardListServlet_Ann extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		boardCount = service.getBoardAnnCount(searchMap);
...
//		pageInfo = new PageInfo(page, 10, boardCount, 20); // 게시글이 20개로 보인다.
		list = service.getBoardAnnList(pageInfo, searchMap);
...
//		list = service.getBoardList(pageInfo);
		...
		req.getRequestDispatcher("/views/html/3_boardList_Ann.jsp").forward(req, resp);
	}
